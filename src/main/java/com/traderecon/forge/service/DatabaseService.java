...
public class DatabaseService {
    ...
    @Autowired
    public DatabaseService(TradeRepository tradeRepository,
    ...
    /**
     * Book a trade to database with rollback (dry-run mode).
     *
     * Executes INSERT inside transaction, then rolls back.
     * This simulates production booking without persisting data.
     */
    public void bookTradeWithRollback(TradeRecord record) {
        DefaultTransactionDefinition def = new DefaultTransactionDefinition();
        def.setName("TradeBookingTransaction");
        def.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRES_NEW);
        def.setTimeout(10); // 10 second timeout
        ...
        try {
            ...
            log.info("Would have booked trade: {} (notional: {}, counterparty: {}, type: {})",
                    record.getTradeId(),
                    record.getNotional(),
            ...
            // ALWAYS ROLLBACK (simulation mode)
            transactionManager.rollback(status);
            log.debug("Transaction rolled back for trade: {}", record.getTradeId());
